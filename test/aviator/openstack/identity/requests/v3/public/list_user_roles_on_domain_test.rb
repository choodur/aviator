...
class Aviator::Test
  ...
  describe 'aviator/openstack/identity/requests/v3/public/list_user_roles_on_domain' do
    ...
    def setup_dependencies
      ...
      @keystone.request(:grant_role_to_user_on_domain, :api_version => :v3) do |params|
    ...
    validate_response 'parameters are valid' do
      ...
      response = @keystone.request(:list_user_roles_on_domain, :api_version => :v3) do |params|
        params[:domain_id] = @domain[:id]
        params[:user_id]   = @user[:id]
      end
      ...
      response.body[:roles].map { |r| r[:id] }.must_include @role[:id]
